...
public class GradosDeSeparacionTest {
  public static void main(String[] args) {
      ...
      red.conectar(v1, v2, 1);    //Messi - Murray
      red.conectar(v2, v1, 1);

      red.conectar(v1, v4, 1);    //Messi - Romero
      red.conectar(v4, v1, 1);

      red.conectar(v1, v3, 1);    //Messi - Polanis
      red.conectar(v3, v1, 1);

      red.conectar(v2, v5, 1);    //Murray - Savenia
      red.conectar(v5, v2, 1);

      red.conectar(v2, v4, 1);    //Murray - Romero
      red.conectar(v4, v2, 1);

      red.conectar(v4, v3, 1);    //Romero - Polanis
      red.conectar(v3, v4, 1);

      red.conectar(v4, v7, 1);    //Romero - Blasco
      red.conectar(v7, v4, 1);

      red.conectar(v3, v6, 1);    //Polanis - Cabral  
      red.conectar(v6, v3, 1);

      red.conectar(v5, v7, 1);    //Savenia - Blasco
      red.conectar(v7, v5, 1);

      red.conectar(v7, v8, 1);    //Blasco - Riquelme
      red.conectar(v8, v7, 1);

      red.conectar(v8, v9, 1);    //Riquelme - Bari
      red.conectar(v9, v8, 1);

      red.conectar(v8, v10, 1);   //Riquelme - Duki
